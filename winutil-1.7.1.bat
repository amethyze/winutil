@echo off
title WinUtil
set startupwinutilversion=1.7.1
set prerelease=n
goto initial
...
if exist %USERPROFILE%\AppData\Local\WinUtil\settings.json (goto initialoexist) else (goto initialonotexist)
...
REM variables in order: start as admin, quick folder path, is it first time or not, version
...
curl https://raw.githubusercontent.com/SteveeWasTaken/winutil/main/curlVersion.txt --output curlVersion.txt >nul
if %ERRORLEVEL%==6 (goto start2)
(
set /p webversion=
)<%USERPROFILE%\AppData\Local\WinUtil\curlVersion.txt
del curlVersion.txt
popd
if /i %WEBVERSION% gtr %WINUTILVERSION% (goto needupdate) else (goto start2)
goto start
...
echo  WinUtil - Windows MultiUtilities v%WINUTILVERSION%
...
echo.
echo  [S] Settings And Debugging Tools                            
echo.
...
:settings2
echo You've chosen an option that doesn't
echo exist. Press anything to go back.
pause >nul
goto winutilsettings
